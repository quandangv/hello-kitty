...
echo_task () {
	ids=`task $filter rc.verbose: rc.report.next.columns:id rc.report.next.labels:1 next`
	count=`echo "${ids}" | wc -l`
	if [ $count -gt 0 ]; then 
	...
	current_id=`echo "${ids}" | sed -n $((index))p`
	current_desc=`task $current_id rc.verbose: rc.report.next.columns:description rc.report.next.labels:1 next`
	current_proj=`task $current_id rc.verbose: rc.report.next.columns:project rc.report.next.labels:1 next`
	echo "$current_id" > /tmp/tw_polybar_id
	if [ -z "$current_proj" ]
	then
		echo " $current_desc"
	else
		echo " $current_desc @ $current_proj"
	fi
}
# mark task as done
mark_done() {
